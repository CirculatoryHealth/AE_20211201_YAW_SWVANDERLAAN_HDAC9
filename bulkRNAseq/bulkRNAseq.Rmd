---
title: "Genes of interest in carotid plaques using bulk RNAseq."
...
PROJECTROOT_loc = paste0(ROOT_loc, "/PLINK/analyses/lookups/AE_TEMPLATE")
RESULTS = paste0(PROJECTROOT_loc, "/bulkRNAseq")
PROJECT_loc = paste0(PROJECTROOT_loc, "/bulkRNAseq")
...
```{r Function: installations, echo=FALSE, message=FALSE, warning=FALSE}
...
Collaboration to study gene expression of `r TRAIT_OF_INTEREST` in relation to
...
# Manual option
# target_genes <- c("PCSK9", "COL4A1", "COL4A2", "COL3A", "COL2A", "LDLR", "CD36")
target_genes_extra <- c("SCARF1")
target_genes_extra
...
target_genes <- c(unlist(gene_list$Gene), target_genes_extra)
target_genes
...
2019-12-11 mapped to b37 and Ensembl 87.
...
We can examine the contents of the Athero-Express Biobank dataset to know what
each variable is called, what class (type) it has, and what the variable
description is.

There is an excellent post on this:
<https://www.r-bloggers.com/working-with-spss-labels-in-r/>.
...
# This is derived from the original SPSS code below and adapted for R.
...
> Measurement: This was measured in citrate plasma, at pg/mL using a LUMINEX
> assay.
...
## Athero-Express Biobank Study Baseline Summary
...
with gene symbol, and select the genes of our interest *`r target_genes`*.
...
```{r boxplots_expression, message=FALSE, warning=FALSE}
...
Based on the above graphs we can see that some of the genes are not
measured/available in our dataset. We will remove these from the `target_genes`
list.
...
# for debug
target_genes_qc_replace <- c("IL1B")
...
## Genes of interest vs. plaque and clinical traits
...
### Categorical plaque traits

We correlated the categorical plaque characteristics to gene expression.

**Figure 4: genes of interest plotted over categorical plaque phenotypes**

```{r boxplot_function, fig.width = 12, fig.height = 10}
# Make directory for plots
ifelse(!dir.exists(file.path(QC_loc, "/PlaquePhenotypes_Cat")), 
       dir.create(file.path(QC_loc, "/PlaquePhenotypes_Cat")), 
       FALSE)
PLAQUE_CAT_loc = paste0(QC_loc,"/PlaquePhenotypes_Cat")
...
for(GENE in target_genes_qc){
  ...
  # print(p1)
  # cat(paste0("Saving image for ", GENE,".\n"))
  ggsave(filename = paste0(PLAQUE_CAT_loc, "/", Today, ".",GENE,".expression_vs_cat_plaquephenotypes.png"), plot = last_plot())
  ggsave(filename = paste0(PLAQUE_CAT_loc, "/", Today, ".",GENE,".expression_vs_cat_plaquephenotypes.pdf"), plot = last_plot())
...
### Numerical plaque traits

**Figure 5: genes of interest plotted over numerical plaque phenotypes**

```{r corr_plots, fig.width = 10, fig.height = 7}
# Make directory for plots
ifelse(!dir.exists(file.path(QC_loc, "/PlaquePhenotypes_Num")), 
       dir.create(file.path(QC_loc, "/PlaquePhenotypes_Num")), 
       FALSE)
PLAQUE_NUM_loc = paste0(QC_loc,"/PlaquePhenotypes_Num")
...
for(GENE in target_genes_qc){
  ...
  # print(p1)
  # cat(paste0("Saving image for ", GENE,".\n"))
  ggsave(filename = paste0(PLAQUE_NUM_loc, "/", Today, ".",GENE,".expression_vs_quant_plaque_traits.png"), plot = last_plot())
  ggsave(filename = paste0(PLAQUE_NUM_loc, "/", Today, ".",GENE,".expression_vs_quant_plaque_traits.pdf"), plot = last_plot())
...
**Figure 6: genes of interest plotted over MACE during 30 days and 3 years
follow-up**

```{r boxplot_epmajor, fig.height = 10, fig.width = 5}
# Make directory for plots
ifelse(!dir.exists(file.path(QC_loc, "/ClinicalTraits")), 
       dir.create(file.path(QC_loc, "/ClinicalTraits")), 
       FALSE)
CLINICAL_loc = paste0(QC_loc,"/ClinicalTraits")


# Make directory for plots
ifelse(!dir.exists(file.path(CLINICAL_loc, "/MACE_30days")), 
       dir.create(file.path(CLINICAL_loc, "/MACE_30days")), 
       FALSE)
MACE30days_loc = paste0(CLINICAL_loc,"/MACE_30days")
...
for(GENE in target_genes_qc){
  ...
  # print(p1)
  # cat(paste0("Saving image for ", GENE,".\n"))
  ggsave(filename = paste0(MACE30days_loc, "/", Today, ".",GENE,".expression_vs_MACE_30days.png"), plot = last_plot())
  ggsave(filename = paste0(MACE30days_loc, "/", Today, ".",GENE,".expression_vs_MACE_30days.pdf"), plot = last_plot())
  
  rm(temp, p1 )
}

# Make directory for plots
ifelse(!dir.exists(file.path(CLINICAL_loc, "/MACE_3years")), 
       dir.create(file.path(CLINICAL_loc, "/MACE_3years")), 
       FALSE)
MACE3years_loc = paste0(CLINICAL_loc,"/MACE_3years")
...
for(GENE in target_genes_qc){
  ...
  # print(p1)
  # cat(paste0("Saving image for ", GENE,".\n"))
  ggsave(filename = paste0(MACE3years_loc, "/", Today, ".",GENE,".expression_vs_MACE_3years.png"), plot = last_plot())
  ggsave(filename = paste0(MACE3years_loc, "/", Today, ".",GENE,".expression_vs_MACE_3years.pdf"), plot = last_plot())
...
Since we have multiple groups, while data and residuals are not normally
distributed, we need to use a Kruskal-Wallis test. The assumptions here are
independent samples (yes), and homoscedasticity (yes). No assumption that the
data or residuals have a known distribution.

```{r kruskal_wallis_all}
var_pairs <- crossing(target_genes_qc, c(plaque_phenotypes_cat)) %>%
  setNames(c("genes", "variables"))

d <- expression_wide %>%
  dplyr::select(target_genes_qc, plaque_phenotypes_cat)

kw_results_cat_p <- var_pairs %>%
...
DT::datatable(kw_results_cat_p)

fwrite(kw_results_cat_p, file = paste0(OUT_loc,"/",Today,".results.kruskal_wallis.gene_vs_cat_plaque_traits.txt"))


var_pairs <- crossing(target_genes_qc, c("epmajor.30days", "epmajor.3years")) %>%
  setNames(c("genes", "variables"))

d <- expression_wide %>%
  dplyr::select(target_genes_qc, epmajor.30days, epmajor.3years)

kw_results_clin <- var_pairs %>%
  dplyr::mutate(r.test = purrr::map2(genes, variables, ~ stats::kruskal.test(d[[.x]], d[[.y]])),
                r.test = purrr::map(r.test, broom::tidy)) %>%
  tidyr::unnest(r.test) %>%
  mutate(padj = p.adjust(p.value, method = "holm"))

DT::datatable(kw_results_clin)

fwrite(kw_results_clin, file = paste0(OUT_loc,"/",Today,".results.kruskal_wallis.gene_vs_cat_clin_traits.txt"))
...
If we calculate the correlation coefficients and corresponding p-values for
these correlations, we get the following dataframe. Correlation method used in
this case is Spearman's, since the genes in most case are not nicely normally
distributed.
...
fwrite(corr_results, file = paste0(OUT_loc,"/",Today,".results.spearman.gene_vs_num_plaque_traits.txt"))

```

## Heatmaps for plaque and clinical traits, and genes of interest
...
Note that this heatmap only contains n = 595, while neutrophils and mast cells
are excluded as variables (low sample sizes). This is due to missing values.

<!-- Note that we removed _DUPD1_ given that it's standard deviation = 0.  -->

**Figure 7: correlation heatmap between expression of plaque phenotypes,
secondary events (epmajor), and genes of interest**

```{r heatmap_corr, fig.height = 12, fig.width = 12}
library(tidyverse)
library(magrittr)
...
--------------------------------------------------------------------------------
    ...
    Change log
    * v1.1.0 Major overhaul, including update to WORCS system.
    * v1.0.5 Small bug fix.
    * v1.0.4 Small fix in the Excel sheet. Added a few lines to distinguish between computers. And added a few lines of code to add genes to the list of targets. 
    * v1.0.3 Fixed weight of files (limit of 10Mb per file for templates). 
    * v1.0.2 Add 'plaque vulnerability index', Fixed baseline table, added codes, and results.
...
+-------------------------------------------------------------------------------+
| <sup>© 1979-2021 Sander W. van der Laan \| s.w.vanderlaan-2[at]umcutrecht.nl  |
